...
const KNOWLEDGE_USAGE_GUIDE_COLLAPSED_KEY = 'bidmind:knowledge-base:usage-guide-collapsed:v1';
...
function loadUsageGuideCollapsedPreference() {
  try {
    return localStorage.getItem(KNOWLEDGE_USAGE_GUIDE_COLLAPSED_KEY) === '1';
  } catch {
    return false;
  }
}
...
function collectContentMetrics(content: string) {
  ...
  return {
    ...
    importedAssets: countMatches(text, /bidmind-asset:\/\/imported-images/gi),
...
function KnowledgeBasePage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.bidmind?.knowledgeBase.onEvent(({ document }) => {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(KNOWLEDGE_USAGE_GUIDE_COLLAPSED_KEY, usageGuideCollapsed ? '1' : '0');
    } catch {
      // 忽略本地存储不可用场景
    }
  }, [usageGuideCollapsed]);
  ...
  const loadInitialData = async () => {
    try {
      ...
      const [config, data] = await Promise.all([
        ...
        window.bidmind?.knowledgeBase.list(),
      ]);
  ...
  const loadAnalysis = async (documentId: string, options?: { silent?: boolean }) => {
    try {
      const data = await window.bidmind?.knowledgeBase.readAnalysis(documentId);
  ...
  const createFolder = async () => {
    ...
    try {
      ...
      const folder = await window.bidmind?.knowledgeBase.createFolder(name.trim());
      if (!folder) return;
  ...
  const uploadDocuments = async () => {
    ...
    try {
      setLoading(true);
      const result = await window.bidmind?.knowledgeBase.uploadDocuments(activeFolder.id);
      if (!result?.success) {
  ...
  const renameFolder = async (folderId: string, currentName: string) => {
    ...
    try {
      const folder = await window.bidmind?.knowledgeBase.renameFolder(folderId, name);
      if (!folder) return;
      setIndex((prev) => ({
        ...prev,
  ...
  const deleteFolder = async (folderId: string, folderName: string) => {
    ...
    try {
      const result = await window.bidmind?.knowledgeBase.deleteFolder(folderId);
  ...
  const deleteDocument = async (document: KnowledgeDocument) => {
    ...
    try {
      const result = await window.bidmind?.knowledgeBase.deleteDocument(document.id);
  ...
  const openDocument = async (document: KnowledgeDocument, mode: KnowledgeViewer['mode']) => {
    ...
    try {
      if (mode === 'markdown') {
        ...
        const markdown = await window.bidmind?.knowledgeBase.readMarkdown(document.id);
        ...
        const items = await window.bidmind?.knowledgeBase.readItems(document.id);
  ...
  const startMatching = async (targetDocument = viewer?.document, batchSizeOverride = batchSize, options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const result = await window.bidmind?.knowledgeBase.startMatching(targetDocument.id, batchSizeOverride);
      if (!options?.silent) {
