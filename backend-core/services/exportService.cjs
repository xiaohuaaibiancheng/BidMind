...
const NUMBERING_FORMAT_MAP = {
  decimal: LevelFormat.DECIMAL,
  'upper-roman': LevelFormat.UPPER_ROMAN,
  'lower-letter': LevelFormat.LOWER_LETTER,
};
let activeExportStyle = { ...DEFAULT_EXPORT_STYLE };
...
function toHalfPoint(pt) {
  return Math.round(clampNumber(pt, 8, 64, 12) * 2);
}

function normalizeExportStyleOptions(raw = {}) {
  return {
    presetId: String(raw.presetId || DEFAULT_EXPORT_STYLE.presetId),
    titleFont: String(raw.titleFont || DEFAULT_EXPORT_STYLE.titleFont),
    titleSize: clampNumber(raw.titleSize, 10, 48, DEFAULT_EXPORT_STYLE.titleSize),
    headingFont: String(raw.headingFont || DEFAULT_EXPORT_STYLE.headingFont),
    headingSize: clampNumber(raw.headingSize, 10, 36, DEFAULT_EXPORT_STYLE.headingSize),
    bodyFont: String(raw.bodyFont || DEFAULT_EXPORT_STYLE.bodyFont),
    bodySize: clampNumber(raw.bodySize, 8, 28, DEFAULT_EXPORT_STYLE.bodySize),
    lineSpacing: clampNumber(raw.lineSpacing, 1, 2.4, DEFAULT_EXPORT_STYLE.lineSpacing),
    numberingFormat: NUMBERING_FORMAT_MAP[String(raw.numberingFormat || '')] ? String(raw.numberingFormat) : DEFAULT_EXPORT_STYLE.numberingFormat,
  };
}

function textRun(text, options = {}) {
  const font = options.font || activeExportStyle.bodyFont;
  const size = options.size ? toHalfPoint(options.size / 2) : toHalfPoint(activeExportStyle.bodySize);
  return new TextRun({
    text: cleanText(text),
    font,
    size,
...
function paragraph(children, options = {}) {
  const line = Math.round((activeExportStyle.lineSpacing || 1.5) * 240);
  return new Paragraph({
    ...
    spacing: { before: options.before || 0, after: options.after ?? 160, line },
...
async function addOutlineItems(children, items, context, level = 1) {
  for (const item of items || []) {
    ...
    const headingSizeHalfPoint = Math.max(
      toHalfPoint(activeExportStyle.headingSize) - Math.max(0, level - 1) * 2,
      toHalfPoint(activeExportStyle.bodySize) + 2
    );
    children.push(paragraph([textRun(title, {
      bold: true,
      font: activeExportStyle.headingFont,
      size: headingSizeHalfPoint,
    })], {
...
function createNumberingConfig(context) {
  ...
  const numberingFormat = NUMBERING_FORMAT_MAP[activeExportStyle.numberingFormat] || LevelFormat.DECIMAL;
...
async function buildDocxResult(payload, options = {}) {
  const prevExportStyle = activeExportStyle;
  activeExportStyle = normalizeExportStyleOptions(payload?.export_style_options || payload?.exportStyleOptions || {});
  ...
  try {
    const children = [
      ...
      paragraph([textRun(payload.project_name || '投标技术文件', {
        bold: true,
        font: activeExportStyle.titleFont,
        size: toHalfPoint(activeExportStyle.titleSize),
    ...
    const doc = new Document({
      ...
      styles: {
        default: {
          document: {
            run: { font: activeExportStyle.bodyFont, size: toHalfPoint(activeExportStyle.bodySize) },
            paragraph: { spacing: { line: Math.round((activeExportStyle.lineSpacing || 1.5) * 240), after: 160 } },
          },
        },
      },
      sections: [{
    ...
    activeExportStyle = prevExportStyle;
  }
}
