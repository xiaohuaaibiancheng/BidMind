...
function createDuplicateCheckService({ app, configStore, workspaceStore } = {}) {
  ...
  function resolveProjectId(payload = {}) {
    return normalizeProjectId(payload.project_id || '');
  }

  function createRunKey(signature, projectId = '') {
    return `${projectId || '__global__'}::${signature}`;
  }

  function loadState(projectId = '') {
    return workspaceStore.loadDuplicateCheck(projectId) || {};
  }

  function updateState(partial, projectId = '') {
    return workspaceStore.updateDuplicateCheck(partial, projectId);
  }

  function emit(webContents, state, projectId = '') {
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('duplicate-check:event', { duplicateCheck: state, project_id: projectId || '' });
    }
  }

  function isCurrentDuplicateCheckSignature(signature, projectId = '') {
  ...
  function updateAnalysis(partial, webContents, signature, projectId = '') {
    if (!isCurrentDuplicateCheckSignature(signature, projectId)) return null;
    const prev = loadState(projectId);
    const prevAnalysis = prev.metadataAnalysis || {};
    const metadataAnalysis = { ...prevAnalysis, ...partial, updated_at: now() };
    const next = updateState({ metadataAnalysis }, projectId);
    emit(webContents, next, projectId);
    return next;
  }

  function updateOutlineAnalysis(partial, webContents, signature, projectId = '') {
    if (!isCurrentDuplicateCheckSignature(signature, projectId)) return null;
    const prev = loadState(projectId);
    const prevAnalysis = prev.outlineAnalysis || {};
    const outlineAnalysis = { ...prevAnalysis, ...partial, updated_at: now() };
    const next = updateState({ outlineAnalysis }, projectId);
    emit(webContents, next, projectId);
    return next;
  }

  function updateContentAnalysis(partial, webContents, signature, projectId = '') {
    if (!isCurrentDuplicateCheckSignature(signature, projectId)) return null;
    const prev = loadState(projectId);
    const prevAnalysis = prev.contentAnalysis || {};
    const contentAnalysis = { ...prevAnalysis, ...partial, updated_at: now() };
    const next = updateState({ contentAnalysis }, projectId);
    emit(webContents, next, projectId);
    return next;
  }

  function updateImageAnalysis(partial, webContents, signature, projectId = '') {
    if (!isCurrentDuplicateCheckSignature(signature, projectId)) return null;
    const prev = loadState(projectId);
    const prevAnalysis = prev.imageAnalysis || {};
    const imageAnalysis = { ...prevAnalysis, ...partial, updated_at: now() };
    const next = updateState({ imageAnalysis }, projectId);
    emit(webContents, next, projectId);
    return next;
  }

  async function runContentExtraction(allFiles, webContents, signature, projectId = '') {
    const config = configStore ? configStore.load() : { file_parser: { provider: 'local' } };
    const dir = path.join(getProjectDuplicateCheckDir(app, projectId) || getDuplicateCheckDir(app), 'contents');
    await fs.mkdir(dir, { recursive: true });
    const results = [];
    const assetScopePrefix = projectId ? `duplicate-check-${projectId}-content` : 'duplicate-check-global-content';
    updateAnalysis({ contentExtraction: { status: 'running', completed: 0, total: allFiles.length }, message: '正在提取正文内容' }, webContents, signature, projectId);

    for (const file of allFiles) {
      const fileId = stableFileId(file);
      try {
        const markdown = (await parseDocumentWithConfig(app, file.file_path, config, {
          assetScope: `${assetScopePrefix}-${fileId}`,
          preserveImages: true,
        })).trim();
      ...
      updateAnalysis({ contentExtraction: { status: 'running', completed: results.length, total: allFiles.length }, contentFiles: results, message: `正文内容提取 ${results.length}/${allFiles.length}` }, webContents, signature, projectId);
    }

    const status = results.some((item) => item.status === 'error') ? 'error' : 'success';
    updateAnalysis({ contentExtraction: { status, completed: results.length, total: allFiles.length }, contentFiles: results }, webContents, signature, projectId);
    return results;
  }

  async function runMetadataExtraction(bidFiles, webContents, signature, projectId = '') {
    const results = [];
    updateAnalysis({ metadataExtraction: { status: 'running', completed: 0, total: bidFiles.length }, message: '正在提取投标文件元数据' }, webContents, signature, projectId);
    ...
    for (const file of bidFiles) {
      ...
      updateAnalysis({ metadataExtraction: { status: 'running', completed: results.length, total: bidFiles.length }, files: results, rows, message: `元数据提取 ${results.length}/${bidFiles.length}` }, webContents, signature, projectId);
    }

    const rows = buildRows(results);
    const status = results.some((item) => item.status === 'error') ? 'error' : 'success';
    updateAnalysis({ metadataExtraction: { status, completed: results.length, total: bidFiles.length }, files: results, rows }, webContents, signature, projectId);
    return results;
  }
  ...
  async function runOutlineAnalysis(tenderFile, bidFiles, contentFiles, signature, webContents, projectId = '') {
    updateOutlineAnalysis({ status: 'running', progress: 5, extraction: { status: 'running', completed: 0, total: bidFiles.length }, message: '正在准备目录分析' }, webContents, signature, projectId);
    const results = [];
    let tenderSentences = [];
    if (tenderFile) {
      try {
        ...
        updateOutlineAnalysis({ message: `招标文件句子白名单生成失败，继续对比投标文件目录：${error.message || error}` }, webContents, signature, projectId);
      }
    }

    updateOutlineAnalysis({ tenderSentenceCount: tenderSentences.length, message: '正在提取投标文件目录' }, webContents, signature, projectId);
    for (const file of bidFiles) {
      const fileId = stableFileId(file);
      try {
      ...
      }, webContents, signature, projectId);
    }
  ...
  async function runContentDuplicateAnalysis(tenderFile, bidFiles, contentFiles, signature, webContents, projectId = '') {
    updateContentAnalysis({ status: 'running', progress: 5, extraction: { status: 'running', completed: 0, total: bidFiles.length }, message: '正在准备正文比对' }, webContents, signature, projectId);
    ...
    if (tenderFile) {
      try {
        ...
        updateContentAnalysis({ message: `招标文件句子白名单生成失败，继续比对投标正文：${error.message || error}` }, webContents, signature, projectId);
      }
    }
    ...
    for (let index = 0; index < bidFiles.length; index += 1) {
      const file = bidFiles[index];
      const fileId = stableFileId(file);
      try {
        const markdown = await readContentMarkdown(contentFiles, file);
        ...
        updateContentAnalysis({ message: `${file.file_name} 正文比对失败：${error.message || error}` }, webContents, signature, projectId);
      }

      updateContentAnalysis({
        status: 'running',
        progress: bidFiles.length ? Math.round((globalSentences.size ? 10 : 5) + ((index + 1) / bidFiles.length) * 80) : 85,
        tenderSentenceCount: tenderSentenceSet.size,
        tenderMatchedSentenceCount,
        totalSentenceCount,
        extraction: { status: 'running', completed: index + 1, total: bidFiles.length },
        message: `正文比对 ${index + 1}/${bidFiles.length}`,
      }, webContents, signature, projectId);
    }
  ...
  async function runImageDuplicateAnalysis(bidFiles, contentFiles, signature, webContents, projectId = '') {
    updateImageAnalysis({ status: 'running', progress: 5, extraction: { status: 'running', completed: 0, total: bidFiles.length }, message: '正在准备图片比对' }, webContents, signature, projectId);
    const results = [];
  ...
  async function run(signature, payload, webContents, projectId = '') {
    const runKey = createRunKey(signature, projectId);
    ...
    try {
      const contentPromise = runContentExtraction(allFiles, webContents, signature, projectId);
      const metadataFiles = await runMetadataExtraction(bidFiles, webContents, signature, projectId);
      updateOutlineAnalysis({ status: 'running', progress: 1, message: '元数据提取完成，等待正文内容用于目录分析', extraction: { status: 'running', completed: 0, total: bidFiles.length } }, webContents, signature, projectId);
      updateContentAnalysis({ status: 'running', progress: 1, message: '元数据提取完成，等待正文内容用于正文比对', extraction: { status: 'running', completed: 0, total: bidFiles.length } }, webContents, signature, projectId);
      updateImageAnalysis({ status: 'running', progress: 1, message: '元数据提取完成，等待正文内容用于图片比对', extraction: { status: 'running', completed: 0, total: bidFiles.length } }, webContents, signature, projectId);
      const contentFiles = await contentPromise;
      const [outlineFiles, contentResult, imageResult] = await Promise.all([
        runOutlineAnalysis(tenderFile, bidFiles, contentFiles, signature, webContents, projectId),
        runContentDuplicateAnalysis(tenderFile, bidFiles, contentFiles, signature, webContents, projectId),
        runImageDuplicateAnalysis(bidFiles, contentFiles, signature, webContents, projectId),
      ]);
      ...
      updateAnalysis({ status: failed ? 'error' : 'success', progress: 100, message: failed ? '部分文件分析失败' : '元数据分析完成' }, webContents, signature, projectId);
    } catch (error) {
      updateAnalysis({ status: 'error', progress: 100, message: error.message || '元数据分析失败' }, webContents, signature, projectId);
    } finally {
  ...
  return {
    startMetadataAnalysis(payload = {}, webContents) {
      ...
      if (!force
        ...
        emit(webContents, current, projectId);
        return current.metadataAnalysis;
      }
      if (!force && running.has(runKey)) {
        emit(webContents, current, projectId);
      ...
      const next = updateState({ tenderFile: payload.tenderFile || null, bidFiles, metadataAnalysis, outlineAnalysis, contentAnalysis, imageAnalysis }, projectId);
      emit(webContents, next, projectId);
      const promise = run(signature, payload, webContents, projectId);
