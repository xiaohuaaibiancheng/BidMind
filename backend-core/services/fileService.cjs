...
function createAssetContext(app, scope = 'documents') {
  ...
  return {
    ...
    urlPrefix: `bidmind-asset://imported-images/${encodeURIComponent(batchId)}`,
    index: 0,
  };
}
...
async function resolveImageToAssetUrl(source, assets, context = {}) {
  ...
  if (/^bidmind-asset:\/\//i.test(value)) return value;
...
module.exports = {
  createFileService,
  duplicateCheckSupportedExtensions,
  localSupportedExtensions,
  mineruAccurateSupportedExtensions,
  mineruAgentSupportedExtensions,
